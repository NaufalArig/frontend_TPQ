...
export default function RiwayatAbsensiTable({ studyClassId, onError }: Props) {
    ...
    useEffect(() => {
        ...
        void fetchData();
    }, [attendanceDate, onError, status, studyClassId]);
    ...
    if (loading) {
        return <p>Loading riwayat absensi...</p>;
    }
        ...
        <>
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <input
                    ...
                    className="w-full rounded-lg border border-gray-300 px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-brand-500 lg:w-80"
                />
            </div>
                    ...
                    <div className="min-w-[1050px]">
                        <Table className="table-fixed text-sm">
                            <TableHeader className="border-b border-brand-300 bg-brand-100">
                                <TableRow>
                                    <TableCell isHeader className="w-[70px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        No
                                    </TableCell>
                                    <TableCell isHeader className="w-[150px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        Tanggal
                                    </TableCell>
                                    <TableCell isHeader className="w-[250px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        <SortableHeader
                                            label="Nama Santri"
                                            sortKey="student"
                                            activeKey={sortKey}
                                    ...
                                    <TableCell isHeader className="w-[170px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        Kelas
                                    </TableCell>
                                    <TableCell isHeader className="w-[130px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        Status
                                    </TableCell>
                                    <TableCell isHeader className="w-[180px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        Keterangan
                                    </TableCell>
                                    <TableCell isHeader className="w-[150px] px-4 py-3 text-start text-theme-xs font-semibold text-black dark:text-gray-400">
                                        Diinput
                                ...
                                {sortedData.length === 0 ? (
                                    ...
                                    paginatedData.map((item, index) => (
                                        <TableRow key={item.id}>
                                            <TableCell className="whitespace-nowrap px-4 py-3 text-theme-sm text-gray-500 dark:text-gray-400">
                                                {pageStart + index + 1}
                                            </TableCell>
                                            <TableCell className="whitespace-nowrap px-4 py-3 text-theme-sm text-gray-500 dark:text-gray-400">
